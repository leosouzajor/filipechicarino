...
const ContatoPage = () => {
  ...
  return <>
      <Helmet>
        <title>Contato | Filipe Chicarino</title>
        <meta name="description" content="Entre em contato com Filipe Chicarino para palestras, treinamentos e consultorias." />
      </Helmet>
                ...
                <div className="bg-white p-8 md:p-10 rounded-[8px] shadow-[0_4px_12px_rgba(0,0,0,0.08)]">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                      ...
                      <input type="text" name="nome" value={formData.nome} onChange={handleChange} className={`w-full px-4 py-3 border rounded-[8px] focus:outline-none focus:ring-2 focus:ring-[#C85A3A] transition-all bg-gray-50 ${errors.nome ? 'border-red-500' : 'border-gray-200'}`} placeholder="Seu nome" />
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <input type="email" name="email" value={formData.email} onChange={handleChange} className={`w-full px-4 py-3 border rounded-[8px] focus:outline-none focus:ring-2 focus:ring-[#C85A3A] transition-all bg-gray-50 ${errors.email ? 'border-red-500' : 'border-gray-200'}`} placeholder="seu@email.com" />
                        {errors.email && <p className="text-red-500 text-sm mt-1">{errors.email}</p>}
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-[#001F3F] mb-2">Empresa</label>
                        <input type="text" name="empresa" value={formData.empresa} onChange={handleChange} className="w-full px-4 py-3 border border-gray-200 rounded-[8px] focus:outline-none focus:ring-2 focus:ring-[#C85A3A] transition-all bg-gray-50" placeholder="Nome da empresa" />
                      </div>
                    </div>

                    <div>
                      <label className="block text-sm font-semibold text-[#001F3F] mb-2">Assunto *</label>
                      <input type="text" name="assunto" value={formData.assunto} onChange={handleChange} className={`w-full px-4 py-3 border rounded-[8px] focus:outline-none focus:ring-2 focus:ring-[#C85A3A] transition-all bg-gray-50 ${errors.assunto ? 'border-red-500' : 'border-gray-200'}`} placeholder="Sobre o que vamos falar?" />
                    ...
                    <div>
                      ...
                      <textarea name="mensagem" rows="5" value={formData.mensagem} onChange={handleChange} className={`w-full px-4 py-3 border rounded-[8px] focus:outline-none focus:ring-2 focus:ring-[#C85A3A] transition-all bg-gray-50 resize-none ${errors.mensagem ? 'border-red-500' : 'border-gray-200'}`} placeholder="Descreva como podemos ajudar..."></textarea>
                      {errors.mensagem && <p className="text-red-500 text-sm mt-1">{errors.mensagem}</p>}
                    </div>

                    <button type="submit" disabled={isSubmitting} className="w-full bg-[#C85A3A] text-white px-8 py-4 rounded-[8px] font-semibold text-lg hover:bg-[#B84A2A] transition-all duration-300 flex items-center justify-center gap-2 shadow-md hover:shadow-xl disabled:opacity-70 disabled:cursor-not-allowed">
